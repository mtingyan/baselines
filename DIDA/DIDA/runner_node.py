...
class DataSplitter:
    ...
    def load_or_create_splits(self):
        ...
        return splits
class Runner(object):
    def __init__(self, args, model, dataset, train_time, test_time_list, save_path, writer=None, **kwargs):
    ...
    def train(self, epoch, data, train_time, test_time_list):
        ...
        splitter = DataSplitter(train_data, args.dataset, self.save_path, 0)
        splits = splitter.load_or_create_splits()
        train_edge_mask = splits["train_edge_mask"]
        train_node_mask = splits["train_mask"]
        edge_index_list = [train_data.edge_index[:,splits["train_edge_mask"]].to(args.device), train_data.edge_index[:,~splits["train_edge_mask"]].to(args.device)]
        ...
        for i, period in enumerate(test_time_list):
            ...
            splitter = DataSplitter(test_data, args.dataset, self.save_path, 0)
            splits = splitter.load_or_create_splits()
            edge_index_list.append(test_data.edge_index[:,~splits["train_edge_mask"]].to(args.device))
        ...
        val_acc = 0
        score_list = {}
        for i, period in enumerate([train_time] + test_time_list):
            z = cs[i+1]
            test_data = data.build_graph(period[0], period[1])
            splitter = DataSplitter(test_data, args.dataset, self.save_path, 0)
            splits = splitter.load_or_create_splits()
            logits = self.loss.predict_label(z, self.model.cs_decoder)
        ...
        edge_index.append(train_data.edge_index[:,train_edge_mask].to(args.device))
        node_label.append(train_data.y.to(args.device)[train_node_mask].squeeze(1))
        tsize.append(node_label[0].size(0))
        ...
        cy = cal_y(cs, self.model.cs_decoder)[train_node_mask, :]
        sy = cal_y(ss, self.model.ss_decoder)[train_node_mask, :]

        conf_loss = cal_loss(sy, node_label)
        causal_loss = cal_loss(cy, node_label)
        ...
        if intervention_times > 0 and la > 0:
            if args.intervention_mechanism == 0:
                ...
                for i in range(intervention_times):
                    ...
                    env_loss = torch.cat(
                        [env_loss,
                         cal_loss(conf, node_label).unsqueeze(0)])
                ...
                node_label = node_label[idxs].split(tsize)
                ...
                for j, t in tqdm(enumerate(tsize)):
                    ...
                    conf = allc * alls
                    alle = node_label[j].expand(intervention_times,
                                                node_label[j].shape[0])
                ...
                conf=allc*alls
                alle=node_label.expand(intervention_times,node_label.shape[0])
                crit=torch.nn.CrossEntropyLoss()
        ...
        return average_epoch_loss, val_acc, score_list
    ...
    def run(self):
        ...
        with tqdm(range(1, args.max_epoch + 1)) as bar:
            for epoch in bar:
                t0 = time.time()
                epoch_loss, val_acc, test_acc_list = self.train(
                ...
                if epoch == 1 or epoch % self.args.log_interval == 0:
                    print(f"Epoch {epoch+1}/{args.max_epoch}, Training Loss: {epoch_loss}, Validation acc: {val_acc}")
