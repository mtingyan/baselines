...
from add_utils import is_directed, generate_node_times, bucketize_and_concat, add_degree_features, add_time_features

class DataSplitter:
    def __init__(self, data, dataname, test_negative_sampling_ratio, save_path):
        self.data = data
...
class EvolvingDataset:
    def __init__(self, dataset, data_path, input_dim, time_dim, degree_dim):
        self.dataset = dataset
        self.data_path = data_path
        self.edges = []
        self.edge_times = {}
        self.feature_dimension = input_dim
        self.time_dim = time_dim
        self.degree_dim = degree_dim
        self.load_data()
    ...
    def build_graph(self, start_time, end_time):
        if self.dataset in [
            ...
            edge_features = torch.tensor(
                np.array(self.edge_feature)[mask], dtype=torch.float32
            )
