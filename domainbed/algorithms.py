...
class ERM(Algorithm):
    ...
    def predict(self, x, edge):
        return self.network(x, edge)

class ERM_GNN(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_GNN, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.network = networks.NodeClassifier(input_shape, self.hparams)
        self.optimizer = torch.optim.Adam(
            self.network.parameters(),
            lr=self.hparams["lr"],
            weight_decay=self.hparams['weight_decay']
    ...
    def update(self, data_list, num_envs):
        for i in range(num_envs):
            if i == 0:
                all_y = data_list[i][0].y
                all_predict = self.predict(data_list[i][0].x, data_list[i][1])
            else:
                all_y = torch.cat(all_y, data_list[i][0].y)
                all_predict = torch.cat(all_predict, self.predict(data_list[i][0].x, data_list[i][1]))
        loss = F.cross_entropy(all_predict, all_y)
...
class ERM_GNN_LR(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_GNN_LR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.network = networks.LinkPredictor(input_shape, self.hparams)
        self.optimizer = torch.optim.Adam(
            self.network.parameters(),
            lr=self.hparams["lr"],
            weight_decay=self.hparams['weight_decay']
    ...
    def update(self, data_list, num_envs):
        ...
        for i in range(num_envs):
            z = self.network.encode(data_list[i][0].x, data_list[i][1])
            edge_index = torch.cat([data_list[i][1], data_list[i][2]], dim=-1).long()
            link_logits = self.network.decode(z, edge_index)
            if i == 0:
                all_logic = link_logits
                all_labels = data_list[i][3]
            else:
                all_logic = torch.cat(all_logic, link_logits)
                all_labels = torch.cat(all_labels, data_list[i][3])
        loss = self.criterion(all_logic, all_labels)
    ...
    def predict(self, data, train_edge_index, train_edge_features, val_edge_index, val_neg_edge_index):
        val_z = self.network.encode(data.x, train_edge_index, train_edge_features)
        edge_index = torch.cat([val_edge_index, val_neg_edge_index], dim=-1).long()
        val_link_logits = self.network.decode(val_z, edge_index)
        return val_link_logits


class IRM_GNN_LR(ERM_GNN_LR):
    """Invariant Risk Minimization"""

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(IRM_GNN_LR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
        self.loss_fn = nn.BCEWithLogitsLoss()
    @staticmethod
    def _irm_penalty(logits, y, loss_fn):
        device = "cuda"
        scale = torch.tensor(1.).to(device).requires_grad_()
        loss_1 = loss_fn(logits[::2] * scale, y[::2])
        loss_2 = loss_fn(logits[1::2] * scale, y[1::2])
    ...
    def update(self, data_list, num_envs):
        ...
        for i in range(num_envs):
            z = self.network.encode(data_list[i][0].x, data_list[i][1], data_list[i][4])
            edge_index = torch.cat([data_list[i][1], data_list[i][2]], dim=-1).long()
            link_logits = self.network.decode(z, edge_index)
            nll += self.criterion(link_logits, data_list[i][3])
            penalty += self._irm_penalty(link_logits, data_list[i][3], self.loss_fn)
        nll /= num_envs
        penalty /= num_envs
...
class IRM_GNN(ERM_GNN):
    """Invariant Risk Minimization"""

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(IRM_GNN, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
    ...
    def update(self, data_list, num_envs):
        ...
        all_logits_idx = 0
        for i in range(num_envs):
            logits = self.network(data_list[i][0].x, data_list[i][1])
            # pdb.set_trace()
            y = data_list[i][0].y[data_list[i][2]].squeeze()
            nll += F.cross_entropy(logits[data_list[i][2]], y)
            penalty += self._irm_penalty(logits[data_list[i][2]], y)
        nll /= num_envs
        penalty /= num_envs
        loss = nll + (penalty_weight * penalty)
...
class VREx_GNN(ERM_GNN):
    """V-REx algorithm from http://arxiv.org/abs/2003.00688"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(VREx_GNN, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
...
class VREx_GNN_LR(ERM_GNN_LR):
    """V-REx algorithm from http://arxiv.org/abs/2003.00688"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(VREx_GNN_LR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
...
class GroupDRO_GNN(ERM_GNN):
    """
    Robust ERM minimizes the error at the worst minibatch
    Algorithm 1 from [https://arxiv.org/pdf/1911.08731.pdf]
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GroupDRO_GNN, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)
        self.register_buffer("q", torch.Tensor())

    # def update(self, minibatches, unlabeled=None):
    def update(self, data_list, num_envs):
        self.q = torch.ones(num_envs)
        losses = torch.zeros(num_envs)
        for i in range(num_envs):
            logits = self.network(data_list[i][0].x, data_list[i][1])
            # pdb.set_trace()
...
class GroupDRO_GNN_LR(ERM_GNN_LR):
    """
    Robust ERM minimizes the error at the worst minibatch
    Algorithm 1 from [https://arxiv.org/pdf/1911.08731.pdf]
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GroupDRO_GNN_LR, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)
        self.register_buffer("q", torch.Tensor())
