...
def accuracy_gnn_class_cls(network, data, edge, mask, device):
    correct = 0
    total = 0
    weights_offset = 0
    weights = None
    ...
    with torch.no_grad():
        ...
        data = data.to(device)
        y = data.y.to(device)
        p = network.predict(data.x, edge)
        y = y[mask]
        p = p[mask]
        _, predicted_classes = torch.max(p, dim=1)  # predicted_classes 是大小为 (N,) 的张量，包含预测类别

        # 2. 比较预测类别与真实标签是否相同
        correct = (predicted_classes == y.squeeze()).sum()  # y.squeeze() 去除维度，保证与 predicted_classes 比较

        accuracy = correct.float() / y.size(0) 
    network.train()

    return accuracy.item()

def accuracy_gnn_lr(network, data, train_edge_index, train_edge_features, val_edge_index, val_neg_edge_index, val_link_labels):
    network.eval()
    criterion = torch.nn.BCEWithLogitsLoss()
    with torch.no_grad():
        # pdb.set_trace()
        val_link_logits = network.predict(data, train_edge_index, train_edge_features, val_edge_index, val_neg_edge_index)
