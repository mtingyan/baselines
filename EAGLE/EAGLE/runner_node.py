...
class Runner(object):
    def __init__(self, args, model, cvae, dataset, train_time, test_time_list, save_path, writer=None, **kwargs):
        ...
        all_edges = train_data.edge_index
        all_time = train_data.edge_time
        node_time = train_data.node_time

        time_min, time_max = 38, all_time.max().item()
        time_bins = torch.linspace(time_min, time_max, 4)
        time_intervals = torch.bucketize(all_time, time_bins)  
        node_time_intervals = torch.bucketize(node_time, time_bins)   
        ...
        for i in range(4):
            ...
            node_mask = (node_time_intervals.squeeze(-1) == i)
        ...
        self.node_mask_list = [node_mask for node_mask in node_mask_list]
    ...
    def train(self, epoch, data, train_time, test_time_list):
        ...
        splitter = DataSplitter(self.args, train_data)
        splits = splitter.load_or_create_splits()
        labels = train_data.y.to(args.device)[self.node_mask_list[-1]].squeeze(1)
        val_score, _ = self.loss.predict_node(z[self.node_mask_list[-1]], labels)
        ...
        for t in range(3):
            z = embeddings[t]
            pos_edge_index = self.edge_index_list_pre[t+1]
        ...
        node_labels = train_data.y.squeeze(1).to(device)
        def cal_node_loss(embeddings, mask_list):
            all_node_loss = 0
            for t in range(3):
                z = embeddings[t]
                mask = mask_list[t]
                all_node_loss += node_criterion(z[mask], node_labels[mask])
            return all_node_loss/3
             
        node_loss = cal_node_loss(embeddings, self.node_mask_list)
        ...
        if epoch % args.every_epoch == 0:
            loss = node_loss + 0.5*main_loss + alpha * var_loss + beta * cvae_loss
        else:
            loss = node_loss + 0.5*main_loss + alpha * var_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    def test(self, epoch, data, time_list):
        ...
        for i, period in enumerate(time_list):
            ...
            for t in range(4 + i):
                ...
                graph = to_networkx(graph_data)
                sampler = NeibSampler(graph, self.nbsz)
                neighbors = sampler.sample().to('cpu')
                neighbors_all.append(neighbors)
            neighbors_all = torch.stack(neighbors_all).to('cpu')
            self.model = self.model.to('cpu')
            embeddings = self.model(
                [
                    edge_index_list_pre[ix].long().to('cpu')
                    for ix in range(4 + i)
                ],
                x_list,
                neighbors_all.to('cpu'),
            )
            node_mask = splits["test_mask"].to('cpu')
            z = embeddings[-1]
            labels = test_data.y.to('cpu')[node_mask].squeeze(1)
            preds = z.argmax(dim=1)[node_mask]
